...
# QWEN TEMPERATURE SWEEP - BATCH 1
# Comparing qwen2.5:7b-128k vs qwen2.5:14b-128k across temperature range 0.0-1.0
# Configuration: 11 temps (0, 0.1, 0.2, ... 1.0), 500 moves, v7-neutral prompt, 131072 context
...
echo "🚀 Starting BATCH 1: Temperature sweep (0.0-1.0)"
...
for TEMP in "${TEMPS[@]}"; do
  ...
  "$SCRIPT_DIR/trigger-by-name.sh" \
    ...
    $TEMP \
    3072 \
    --comment "BATCH 1 (run $RUN_NUM of $TOTAL_RUNS)"
...
echo ""
echo "✅ BATCH 1 triggered successfully!"
echo "   22 experiments queued (11 temps × 2 models)"
echo ""
echo "Monitor progress:"
echo "  watch -n 5 'psql ... -c \"SELECT * FROM v_batch_monitor WHERE comment LIKE '\'BATCH 1%\'' ORDER BY id;\"'"
