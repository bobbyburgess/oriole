// Bedrock Agent construct for Oriole grid exploration
...
class BedrockAgentConstruct extends Construct {
  constructor(scope, id, props) {
    ...
    const agentProps = {
      ...
      description: `Oriole grid exploration agent using ${modelId}`,
      // Inline action groups (CDK v2.220+)
      actionGroups: [
        {
          actionGroupName: 'oriole-grid-exploration',
          actionGroupExecutor: {
          ...
          description: 'Tools for exploring the 2D grid'
        }
      ]
    };
    ...
    // Create Agent Alias
    // Note: Aliases must be prepared via AWS CLI after agent is prepared
    // CDK creates the alias but doesn't prepare it automatically
    const agentAlias = new bedrock.CfnAgentAlias(this, 'AgentAlias', {
      agentId: agent.attrAgentId,
      agentAliasName: 'prod',
      description: 'Production alias for grid exploration agent'
    });
