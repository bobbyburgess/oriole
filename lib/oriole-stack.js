...
class OrioleStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const actionRouterConcurrency = cdk.Token.asNumber(
      ssm.StringParameter.valueForStringParameter(this, '/oriole/lambda/action-router-concurrency')
    );

    // Step Functions retry configuration
    const retryIntervalSeconds = cdk.Token.asNumber(
      ssm.StringParameter.valueForStringParameter(this, '/oriole/orchestration/retry-interval-seconds')
    );
    const retryMaxAttempts = cdk.Token.asNumber(
      ssm.StringParameter.valueForStringParameter(this, '/oriole/orchestration/retry-max-attempts')
    );
    const retryBackoffRate = cdk.Token.asNumber(
      ssm.StringParameter.valueForStringParameter(this, '/oriole/orchestration/retry-backoff-rate')
    );
    ...
    const actionRouterLambda = new lambda.Function(this, 'ActionRouterFunction', {
      ...
      reservedConcurrentExecutions: actionRouterConcurrency  // Serialize tool calls to simplify rate limiting
    });
    ...
    // For Claude 3.5 Haiku (10 rpm w/ buffer): 7 seconds between requests
