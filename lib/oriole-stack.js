...
class OrioleStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // Step Functions timeout in seconds (CDK limitation: tokens can only use Duration.seconds())
    const maxExecutionSecondsStateMachine = cdk.Token.asNumber(
      ssm.StringParameter.valueForStringParameter(this, '/oriole/experiments/max-execution-seconds')
    );
    ...
    // All database connection parameters are now in Parameter Store for flexibility
    const dbEnvVars = {
      DB_HOST: ssm.StringParameter.valueForStringParameter(this, '/oriole/db/host'),
      DB_PORT: ssm.StringParameter.valueForStringParameter(this, '/oriole/db/port'),
      DB_NAME: ssm.StringParameter.valueForStringParameter(this, '/oriole/db/name'),
      DB_USER: ssm.StringParameter.valueForStringParameter(this, '/oriole/db/user')
    };
    ...
    new ssm.StringParameter(this, 'PromptV1', {
      ...
      stringValue: 'You are navigating a 2D maze on a 60x60 grid. Your goal is to find the target object. You can see 3 blocks in each cardinal direction (line-of-sight: walls block vision). Use move_north, move_south, move_east, move_west to navigate. Use recall_last_25, recall_last_50, recall_last_100, or recall_last_200 to review tiles you have seen (choose based on how much context you want to use).',
    ...
    //  - /oriole/ollama/* (Ollama endpoint, API key, max actions)
    //  - /oriole/viewer/* (viewer UI colors and display settings)
    lambdaRole.addToPolicy(new iam.PolicyStatement({
      ...
      resources: [
        ...
        `arn:aws:ssm:${this.region}:${this.account}:parameter/oriole/ollama/*`,
        `arn:aws:ssm:${this.region}:${this.account}:parameter/oriole/viewer/*`
      ]
    }));
    ...
    // Grant SSM Parameter Store access for Ollama config, prompts, experiment config, and database
    ...
    const invokeAgentOllamaLambda = new lambda.Function(this, 'InvokeAgentOllamaFunction', {
      ...
      code: lambda.Code.fromAsset(path.join(__dirname, '../lambda')),
      description: 'Ollama agent orchestration with performance metrics tracking',
      environment: {
        ...dbEnvVars,
    ...
    // EXECUTION TRACKING: Inject Step Functions execution context into Lambda event
    // This allows us to store execution_arn, execution_name in the DB at experiment START,
    // ensuring we can diagnose failures even if the execution crashes before completion.
    // See: db/migrations/015_add_execution_tracking_columns.sql
    // See: lambda/orchestration/start-experiment.js (executionContext handling)
    const startStep = new tasks.LambdaInvoke(this, 'StartExperiment', {
      lambdaFunction: startExperimentLambda,
      // Inject execution metadata from Step Functions context ($$.Execution.*)
      // This merges the original event with executionContext object
      payload: sfn.TaskInput.fromObject({
        // Spread original event fields (agentId, modelName, etc)
        ...sfn.JsonPath.objectAt('$'),
        // Add execution tracking metadata
        executionContext: {
          // Full ARN: arn:aws:states:REGION:ACCOUNT:execution:STATE_MACHINE:EXEC_NAME
          executionArn: sfn.JsonPath.stringAt('$$.Execution.Id'),
          // Short name: ollama_{uuid}_{uuid} or bedrock_{uuid}_{uuid}
          executionName: sfn.JsonPath.stringAt('$$.Execution.Name'),
          // SQS message ID (if triggered from queue, null otherwise)
          // Note: This assumes the event has a 'messageId' field from SQS
    ...
    // EXECUTION TRACKING: Pass full errorInfo to Lambda for structured error capture
    // errorInfo contains: { Error: "Lambda.Timeout", Cause: "Task timed out..." }
    // Lambda uses this to set execution_status (FAILED vs TIMED_OUT) and populate last_error
    // See: lambda/orchestration/finalize-experiment.js (ERROR PATH section)
    const finalizeOnError = new tasks.LambdaInvoke(this, 'FinalizeOnError', {
      ...
      payload: sfn.TaskInput.fromObject({
        ...
        // Pass full errorInfo for structured error capture in DB
        // This allows Lambda to distinguish Lambda.Timeout from States.TaskFailed, etc.
    ...
    // Cooldown wait - Delay for model switches when insufficient time has elapsed
    // Duration from Parameter Store: /oriole/experiments/cooldown-seconds (default: 10s)
    // Only applied when start-experiment returns needsCooldown=true (model switch detection)
    const cooldownWait = new sfn.Wait(this, 'CooldownWait', {
      time: sfn.WaitTime.duration(cdk.Duration.seconds(cooldownSeconds))
    });

    // Smart cooldown router - Only delays for model switches with insufficient elapsed time
    // Checks needsCooldown flag from start-experiment Lambda
    // If true: Apply cooldown wait (model switched, still warm)
    // If false: Skip directly to agent router (same model or already cool)
    const cooldownChoice = new sfn.Choice(this, 'NeedsCooldown?')
      .when(
        sfn.Condition.booleanEquals('$.needsCooldown', true),
        cooldownWait.next(agentRouterChoice)
      )
      .otherwise(agentRouterChoice);
    ...
    // Start -> CooldownChoice -> (if needed: Cooldown ->) AgentRouter -> (Bedrock|Ollama) -> CheckProgress -> Choice
    const definition = startStep
      .next(cooldownChoice);

    // Create the state machine
    // Timeout configurable via /oriole/experiments/max-execution-seconds (default: 7200s = 2 hours)
    // Should match or exceed /oriole/max-duration-minutes for graceful exit
    // Note: Requires CDK redeployment to take effect (infrastructure change)
