...
class OrioleStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const invokeOllamaAgentStep = new tasks.LambdaInvoke(this, 'InvokeOllamaAgent', {
      ...
      errors: ['States.TaskFailed'], // Retry on Lambda errors (including network issues)
      interval: cdk.Duration.seconds(5), // Wait 5s before first retry
      maxAttempts: 3, // Try up to 3 times (1 original + 2 retries)
      backoffRate: 2.0 // Double wait time each retry (5s, 10s, 20s)
    }).addCatch(finalizeOnError, {
      errors: ['States.TaskFailed'],
      resultPath: '$.errorInfo'
    });
